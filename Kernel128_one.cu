...
__global__ void kernel_512_one_128(float *A, float *B, float *bnBias, float *bnScale, float *C) {
	...
	for (int k = 0; k < 128; k += 16) {
		for (int i = 0; i < 16; i++)
			weights[ind + i*512] = B[(k+i)*512 + ind];
		__syncthreads();

		float *A_start = input + k*64;
		for (int p = 0; p < 64; p++) {
...
__global__ void kernel_128_one_512(float *A, float *B, float *bnBias, float *bnScale, float *C) {
	...
	for (int k = 0; k < 32; k += 16) {
		for (int i = 0; i < 16; i++)
			weights[ind + 512*i] = B[(k+i)*2048 + part*128 + in_channel + line*512];
		__syncthreads();
	...
	C_start[line * 512 + in_channel] = scale[in_channel] * output[ind] + bias[in_channel];
}
